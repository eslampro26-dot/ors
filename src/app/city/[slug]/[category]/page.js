          ...
          >
            <div style={{
              ...
              <div style={{ padding: 'clamp(1.2rem, 4vw, 2.5rem)', maxWidth: '800px', margin: '0 auto' }}>
                ...
                {/* Book Now */}
                <a
                  href={`/checkout?tripId=${mdTrip.id}&price=${mdTier?.price || mdTrip.price}&titleAr=${encodeURIComponent(mdTiers.length > 1 ? (mdTrip.titleAr + ' - ' + (mdTier?.names?.ar || '')) : mdTrip.titleAr)}&titleEn=${encodeURIComponent(mdTiers.length > 1 ? ((mdTrip.titleEn || mdTrip.titleAr) + ' - ' + (mdTier?.names?.en || '')) : (mdTrip.titleEn || mdTrip.titleAr))}&type=trip&city=${encodeURIComponent(city.nameAr)}&category=${category}&tier=${mdTier?.id || 'economy'}&tierDesc=${encodeURIComponent(mdTier?.descriptions?.[locale] || mdTier?.descriptions?.en || mdTier?.descriptions?.ar || '')}&childPrice=${mdTier?.childPrice ?? mdTrip.childPrice ?? 0}&infantPrice=${mdTier?.infantPrice ?? mdTrip.infantPrice ?? 0}&additionalPersonPrice=${mdTier?.additionalPersonPrice ?? mdTrip.additionalPersonPrice ?? 0}&allowedRequests=${encodeURIComponent(JSON.stringify(mdTrip.specialRequests || []))}`}
                  className="btn btn-primary"
